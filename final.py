...
class PepGenXClient:
    ...
    def chunk_json_file(
        ...
        *,
        ...
        min_chunk_length: int = 0,          # set to 0 to avoid filtering small outputs
        ...
        for attempt in range(self.cfg.max_retries + 1):
            try:
                ...
                if resp.status_code != 200:
                    print(f"[CHUNK ERROR] HTTP {resp.status_code}. Body starts with: {resp.text[:500]!r}", flush=True)
...
def get_pepgenx_client() -> PepGenXClient:
    ...
    # Require these — the API expects them
...
def main() -> None:
    ...
    # Ensure this path is visible to the service (best: run on Pepsi VM)
    server_file_path = r"D:\sujit\JSON_FOR_VDB"
    file_name = "1_SQL.json"

    # Chunking params
    chunking_strategy = os.getenv("CHUNKING_STRATEGY", "recursivejsonsplitter")
    chunk_size = int(os.getenv("CHUNK_SIZE", "800"))
    chunk_overlap = int(os.getenv("CHUNK_OVERLAP", "20"))
    min_chunk_length = int(os.getenv("MIN_CHUNK_LENGTH", "0"))     # 0 for now
    max_chunk_length = int(os.getenv("MAX_CHUNK_LENGTH", "800"))
    num_rows_per_chunk = int(os.getenv("NUM_ROWS_PER_CHUNK", "2"))

    # JQ: default to tostring (simple & robust). Override via env if needed.
    jq_schema = _load_jq_schema(os.getenv("JQ_SCHEMA", "tostring")) or "tostring"
    ...
    if not docs:
        print("[INFO] No chunks returned. This usually means JQ produced no text or content was filtered. "
              "Try JQ='tostring' and min_chunk_length=0 (already set).", flush=True)
